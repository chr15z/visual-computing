...
def evc_demosaic_pattern(input_image: np.ndarray, pattern = 'RGGB') -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    G1 = np.zeros(input_image.shape)
    G2 = np.zeros(input_image.shape)
    G = np.zeros(input_image.shape)
    B = np.zeros(input_image.shape)

    if (pattern == 'RGGB'):
        R[::2, ::2] = input_image[::2, ::2]
        G1[1::2, ::2] = input_image[1::2, ::2]
        G2[::2, 1::2] = input_image[::2, 1::2]
    ...
    if(pattern == 'GRBG'):
        G1[::2, ::2] = input_image[::2, ::2]
        R[1::2, ::2] = input_image[1::2, ::2]
        B[::2, 1::2] = input_image[::2, 1::2]
        G2[1::2, 1::2] = input_image[1::2, 1::2]
...
def evc_interpolate(red : np.ndarray, green : np.ndarray, blue : np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	...
	# NOTE: The following three lines can be removed. They prevent the framework
    #       from crashing.
    filter = np.array([[0.00, 0.25, 0.00],
                        [0.25, 1.00, 0.25],
                        [0.00, 0.25, 0.00]])
    
    filter2 = np.array([[0.25, 0.50, 0.25],
                        [0.50, 1.00, 0.50],
                        [0.25, 0.50, 0.25]])
    

    R_inter = scipy.ndimage.correlate(red, filter2, mode='constant')
    G_inter = scipy.ndimage.correlate(green, filter, mode='constant')
    B_inter = scipy.ndimage.correlate(blue, filter2, mode='constant')
    ### END STUDENT CODE
...
def evc_concat(R: np.ndarray, G: np.ndarray, B: np.ndarray) -> np.ndarray:
    ...
    result = np.stack([R,G,B], axis = 2) #konkatiniere die 3 arrays in dimension 2
    ### END STUDENT CODE
