...
def clip_plane(vertex_count : int, positions : np.ndarray, colors : np.ndarray, plane : ClippingPlane) -> List[np.ndarray]:
    """ clips all vertices defined in positions against the clipping
        ...
        Input Parameter
            vertex_count          ... number of vertices of the face that is clipped / Eckpunkte die geclippt werden
    ...
    for i in range(vertex_count):
        ...
        prev_point = positions[(i - 1) % vertex_count]

        t = plane.intersect(prev_point, current_point)

        #Intersecting point = MeshVertex.mix(prev_point, current_point,t)
        Intersecting_point = MeshVertex.mix(prev_point, current_point,t)
        Intersecting_point_color = MeshVertex.mix(colors[(i - 1) % vertex_count],colors[i], t)

        if plane.inside(current_point):

            if not plane.inside(prev_point):
                pos_clipped[vertex_count_clipped] = Intersecting_point
                col_clipped[vertex_count_clipped] = Intersecting_point_color
                vertex_count_clipped += 1
            pos_clipped[vertex_count_clipped] = current_point
            col_clipped[vertex_count_clipped] = current_color
            vertex_count_clipped += 1
        elif plane.inside(prev_point):
            col_clipped[vertex_count_clipped] = Intersecting_point_color
            pos_clipped[vertex_count_clipped] = Intersecting_point
            vertex_count_clipped += 1
    #col_clipped = colors
